...
minioClient.listBuckets().then(
    ...
    if (bucketNames.indexOf(DUTCHIE_BUCKET_NAME) < 0) {
      minioClient.makeBucket(DUTCHIE_BUCKET_NAME, "us-east-1", (err) => {
        if (err) {
          ...
          logger.error(
            `no information will be saved at the MinIO destination...`
          );
        }
        logger.info(
  ...
  (err) => {
    logger.error(
      `ERROR: minio Client cannot listBuckets on '${MINIO_ENDPOINT}:${MINIO_PORT}'`
    );
    if (err) {
      logger.error(err);
    }
  ...
  return new Promise((resolve, reject) => {
    minioClient.putObject(
      ...
      (err: Error, etag: string) => {
        if (err) {
          logger.warn(
            `error uploading to S3 bucket '${bucketName}', ${err.message}`
          );
          return reject(err);
        }
        logger.info(
