...
export const initialize = async (context: Context) => {
  ...
  for (let i = 0; i < sch.length; i++) {
    ...
    const scheduleableFunctions = integratedFunctions.filter(
      (f) => f.scheduleable
    );
    ...
    if (!fn) {
      logger.warn(
        `tried to schedule function '${functionName}', but not integrated!`
      );
      continue;
    }
    type ReqBodyType = z.TypeOf<typeof fn.schema>;
    const foundQueue = await getQueue<ReqBodyType>(
